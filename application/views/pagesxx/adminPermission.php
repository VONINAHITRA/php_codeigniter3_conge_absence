                  ...
                  <ul class="nav topnav">
                    ...
                    <li class=" dropdown ">
                      ...
                      <ul class="dropdown-menu" style="background-color: #444444 ">
                        ...
                        <li style="background-color:#f5f5f5;"><a href="<?php echo base_url(); ?>control/lienpaterniteAdmin"><font color="#bb2839">Congé de paternité</font></a></li>
                        <li><a href="<?php echo base_url(); ?>control/lienMaterniteAdmin">Congé de maternité</a></li>
                      </ul>
                    </li>
 ...
 <div class="modal fade" tabindex="-1" id="ModalValider" role="dialog" style="background-color: #f4f6f9;">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
     ...
     <!-- removeMember -->
<div class="modal fade" tabindex="-1" role="dialog" id="autoValiderConfirme">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="post" action="" id="myFormValider">
          <input type="hidden" name="editId_autoV" id="editId_autoV">
          <input type="hidden" name="editIm_autoV" id="editIm_autoV">
          <input type="hidden" name="editAnnee_autoV" id="editAnnee_autoV">
          <input type="hidden" name="editnbjrs_autoV" id="editnbjrs_autoV">
        <p>Voulez vous vraiment validé cette demande de congé dd ?</p>
      </div>
      <div class="modal-footer">
        <button style="background-color: #bb2839; color: #fff;" type="button" id="AnBtn" class="btn btn-flat">Annuler</button>
        <button style="background-color: #bb2839;color: #fff;" type="button" id="btnValider" class="btn btn-flat" data-dismiss="modal">OK</button>
      </div>
      </form>
    </div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
...
<div class="modal fade" tabindex="-1" role="dialog" id="confirmeRejeter">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header" style="background-color:#bb2839; ">
        <h5 class="modal-title" style="color: #fff;">Confirmation</h5>
      ...
      <div class="modal-body">
        <form method="post" action="" id="myFormRejeter">
         <input type="hidden" name="editid_autoR" id="editId_autoR">
      ...
      <button style="background-color: #bb2839; color: #fff;" type="button" id="AnnulerBtnRejeter" class="btn btn-flat">Annuler</button>
        <button style="background-color: #bb2839;color: #fff;" type="button" id="btnRejeter" class="btn btn-flat" data-dismiss="modal">OK</button>
      </form>
      </div>
    </div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
...
<div class="modal fade" tabindex="-1" role="dialog" id="confirmeAutoEdit">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header" style="background-color:#bb2839; ">
        <h5 class="modal-title" style="color: #fff;">Confirmation</h5>
      ...
      <div class="modal-body">
        <form method="post" action="" id="myFormEditValider">
         <input type="hidden" name="id_auto" id="editId_autoV">
         <input type="hidden" name="im_auto" id="editIm_autoV">
         <input type="hidden" name="nbjrs_auto" id="editNbjrs_autoV">
         <input type="hidden" name="annee_auto" id="editAnnee_autoV">
      ...
      <div class="modal-footer" style="margin-left: 75%;">
        ...
        <button type="button" id="btnEditValider" class="btn btn-flat" style="margin-right: 16%;background-color: #bb2839;color:#fff;">OK</button>
        </form>
      </div>
    </div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
...
<div class="modal fade" tabindex="-1" role="dialog" id="confirmeSupValider">
        ...
        <div class="row mt-4">
            <nav class="w-150">
              <div class="nav nav-tabs" id="product-tab" role="tablist">
                <a class="nav-item nav-link active" id="product-desc-tab" data-toggle="tab" href="#product-desc" role="tab" aria-controls="product-desc" aria-selected="true" style="font-size: 16px;font-style:;color: #717171;">Demande d'autorisation d'absence</a>
          ...
          <table  class="table table-bordered" id="tabEncours" style="font-size: 13px; width: 100%;">
                <thead>  
                <tr class="tabtr">
              ...
              <div class="tab-pane fade show active" id="product-comments" role="tabpanel" aria-labelledby="product-comments-tab" style="font-size: 13px; width: 100%;"> 
         <table  class="table table-bordered" id="tabValider" style="font-size: 13px; width: 100%;">
                <thead>  
                <tr class="tabtr">
                  <th style=" background: #444444;color: #fff">Matricule</th>
                  <th style=" background: #444444;color: #fff">Titre de congé</th>
              ...
              <div class="tab-pane fade show active" id="product-rating" role="tabpanel" aria-labelledby="product-rating-tab" style="font-size: 13px; width: 100%;">

              <table  class="table table-bordered" id="tabRejeter" style="font-size: 13px; width: 100%;">
                <thead>  
                <tr class="tabtr">
                  <th style=" background: #444444;color: #fff">Matricule</th>
                  <th style=" background: #444444;color: #fff">Titre de congé</th>
  ...
  <footer class="main-footer" style="background-color: #bb2839;color: #fff;">
    ...
    Application pour la gestion des congés et des absences <br>
  </footer>
...
$(document).ready(function() {
  loadTableEncours = $("#tabEncours").DataTable({
    'ajax': '<?php echo base_url();?>control/listePermissionEncours',
    'orders': [],
  }); 
});
...
$(document).ready(function() {
  loadTableValider = $("#tabValider").DataTable({
    'ajax': '<?php echo base_url();?>control/listePermissionValider',
    'orders': [],
  }); 
});
...
$(document).ready(function() {
  loadTableRejeter = $("#tabRejeter").DataTable({
    'ajax': '<?php echo base_url();?>control/listePermissionRejeter',
    'orders': [],
  }); 
});
// nouveau personnel, get url du post
 ...
 //Annuler bouton   myModalEdit   btnValider
$(function(){
 ...
 $('#myFormRejeter').attr('action','<?php echo base_url();?>control/demandeAutorisationRejeter');
  });

      $(function(){
      $('#btnRejeter').on('click',function(){
        //$('#myFormValider')[0].reset();
       var url = $('#myFormRejeter').attr('action');
       var data = $('#myFormRejeter').serialize();
        $.ajax({
       type: 'ajax',
       method: 'post',
       url: url,
       ...
       success: function(data){
        ...
        $('#myModal').modal('hide');
        $('#myFormRejeter')[0].reset();
        loadTableValider.ajax.reload(null, false); 
        toastr.success('<b>Notification</b> </br>Rejeter avec succes').show();
       },
       error: function(){
        $('#confirmeModal').modal('hide');
        $('#myFormRejeter')[0].reset();
 ...
 $('#myFormValider').attr('action','<?php echo base_url();?>control/demandeAutorisationValider');
  });

      $(function(){
      $('#btnValider').on('click',function(){
        //$('#myFormValider')[0].reset();
       var url = $('#myFormValider').attr('action');
       var data = $('#myFormValider').serialize();
        $.ajax({
       type: 'ajax',
       method: 'post',
       url: url,
...
function validerAutorisation(id_auto = null) 
{
  if(id_auto) {

    $.ajax({
      url: '<?php echo base_url();?>control/selectAutoValider'+'/' + id_auto,
      type: 'post',
      dataType: 'json',
      success:function(response) {
        $("#editId_autoV").val(response.id_auto);
        $("#editIm_autoV").val(response.im_auto);
        $("#editAnnee_autoV").val(response.annee_auto);
        $("#editnbjrs_autoV").val(response.nbjr_auto);
      }
    });
  }
  else {
...
function rejeterAutorisation(id_auto = null) 
{
  if(id_auto) {
    ...
    $.ajax({
      ...
      dataType: 'json',
      success:function(response) {
        $("#editId_autoR").val(response.id_auto);
      }
    });
  }
  else {
...
function rejeterConge(id_conge = null) 
{
  if(id_conge) {
    ...
    alert('error');
        ...
        $('#myFormEditValider').attr('action','<?php echo base_url();?>control/autorisationValider');
